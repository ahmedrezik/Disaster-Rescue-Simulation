...
public static void main(String[] args) {
	try {
		Simulator x=new Simulator(null);
		System.out.println(x.buildings.size());
	} catch (Exception e) {
...
public ResidentialBuilding getBuildingByLocation2(Address location) {
	for (int i = 0; i < buildings.size(); i++) {
...
public ArrayList<Citizen> getCitizenByLocation(Address location) {
	ArrayList<Citizen> x=new ArrayList<Citizen>();
	
	for (int i = 0; i < citizens.size(); i++) {
		if (citizens.get(i).getLocation().getX() == location.getX()&&(citizens.get(i).getLocation().getY()==location.getY()))
			 x.add(citizens.get(i));
	}
	return x;
}
}
